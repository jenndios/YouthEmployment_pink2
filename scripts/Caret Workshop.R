...
# want all my data instead of the last column^^
...
#tuneGrid used for us to pick what hyper parameter we want used
...
model_ada <- train(num ~ ., data=trainData, method='ada', trControl = trControl)

model_ada$results

modelnames

model_blackboost <- train(num ~ ., data = trainData, method = 'blackboost', trControl = trControl)
model_blackboost$results

model_AdaBag <- train(num ~ ., data = trainData, method = 'AdaBag', trControl = trControl)
model_AdaBag$results

#Compare the two models using resamples
model_comp <- resamples(list(my_new_model = model_blackboost, Rpart = model_rpart))
summary(model_comp)
